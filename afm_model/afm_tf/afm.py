...
class AFM(BaseEstimator, TransformerMixin):
    ...
    def _init_graph(self):
        ...
        with self.graph.as_default():
            ...
            w = self.weights['attention_w']
            b = self.weights['attention_b']
            self.attention_wx_plus_b = tf.reshape(tf.add(tf.matmul(x,w),b),shape=[-1, num_interactions, self.attention_size])

            res = tf.multiply(tf.nn.relu(self.attention_wx_plus_b),self.weights['attention_h'])
            self.attention_exp = tf.exp(tf.reduce_sum(res,axis=2,keep_dims=True))
    ...
    def _initialize_weights(self):
        ...
        weights['attention_h'] = tf.Variable(
            np.random.normal(loc=0, scale=1, size=(self.attention_size,)),
